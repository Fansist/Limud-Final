...
// THE SAME TWO-UPLOAD FORM the classroom teacher uses, posting to
// /api/self/units instead of /api/teacher/units. Looks intentionally
// identical — the spine is the spine, regardless of role.
...
export function NewSelfUnitForm({
  isDemo,
  defaultRubricJson,
  defaultObjectives,
  defaultDueAt
  ...
  const [sourceHtml, setSourceHtml] = useState<string>(
    "<h2>Topic</h2>\n<p>Source content for the AI to personalize for each kid...</p>"
  );
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>): void {
    ...
    if (isDemo) {
      setDemoBanner(
        "Demo mode: in production this would create the unit inside your district of one. Returning to your dashboard…"
      );
      startTransition(() => {
        setTimeout(() => {
          router.push("/self");
    ...
    startTransition(async () => {
      try {
        const res = await fetch("/api/self/units", {
          method: "POST",
    ...
    <form onSubmit={onSubmit} className="space-y-6">
      <div className="card border-l-4 border-l-brand-500 p-4">
        <p className="text-sm text-ink">
          <strong>Same Assignment for every kid. Different Material for
          each one.</strong>{" "}
          Personalization happens automatically — same engine the rest of
          Limud uses.
        </p>
      </div>
      ...
      <section className="card space-y-3 p-5">
        ...
        <div>
          ...
          <input
            ...
            required
            placeholder="e.g. Photosynthesis intro"
          />
        </div>
        <div>
          <label className="label" htmlFor="description">
            Description (shown to your kids)
          </label>
          <input
            id="description"
            className="input mt-1"
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="card space-y-3 border-t-4 border-t-brand-500 p-5">
          <header>
            ...
            <p className="text-sm text-ink-soft">
              Same questions, same rubric, same points for every kid.
        ...
        <section className="card space-y-3 border-t-4 border-t-accent-warm p-5">
          <header>
            ...
            <p className="text-sm text-ink-soft">
              The source content. Limud rewrites it for each kid, preserving
